...
一部流用や改変などを行う場合の条件は、ゴーストエクスプローラ通の中に含んでいる各ライブラリの条件に従います。
...
* [変更] ゴースト切り替え時や呼び出し時のトークが発生するようにした
         ※ゴーストエクスプローラ通側でSSPの挙動を再現しているため、SSPでの切り替え/呼び出しとは挙動が一部異なるかもしれません
...
* [修正] ゴースト側 descript.txt に seriko.defaultsurfacedirectoryname が指定されていても反映されない不具合を修正
