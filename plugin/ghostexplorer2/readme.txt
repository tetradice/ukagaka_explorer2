...
* [修正] シェルフォルダの中に descript.txt が存在しないゴーストがいると、システムエラーが発生する不具合を修正

2020/9/24 ver 1.2.0 リリース

* [変更] ver 1.1.0で削除した不在判定機能を復帰
         ※ぽなさんからのアドバイスで、不在判定を正しく行う方法が見つかったため

2020/9/22 ver 1.1.0 リリース

* [変更] 不在判定機能を削除。起動中のゴーストについても立ち絵を表示するようにし、「呼び出す」ボタンを無効化しないようにした
         ※同じsakuranameとkeronameを持つゴーストが複数いる場合に、正しく不在判定を行えないという不具合があり、これに対応する方法が見つからないため
* [修正] ゴーストフォルダが隠しフォルダになっている場合にも、ゴーストが表示されてしまう不具合を修正
         ※SSPやmateriaの動作に合わせた
* [修正] 一部ゴーストの立ち絵を正しく処理できない不具合を修正
...
* [修正] シェルエクスプローラ通と同時にインストールしている場合や、複数バージョンを同時にインストールした場合に、エラーで落ちる場合がある不具合を修正
* [内部的変更] ver 0.3.2 以前と同様に、YAYA as PLUGINを使用する構成に戻した（SakuraBridgeを廃止）

2020/9/20 ver 1.0.1 リリース

* [修正] SSP側で2つ以上のゴーストフォルダを登録しており、かつ日本語のフォルダ名が含まれている場合に、そのフォルダが表示されない不具合を修正

2020/9/20 ver 1.0.0 リリース

* [機能追加] オプション画面から、ゴーストエクスプローラ通のreadme.txtを閲覧できるようにした
* [修正] 一部ゴーストの立ち絵を正しく処理できない不具合を修正
...
* [変更] SSP バージョン 2.4.74 以降でのみ、ゴースト切り替え時／呼び出し時のイベントトークが発生するようにした
...
* [修正] 並び順を「最近起動した順」にして、ゴーストフォルダの切り替えを行った場合に、その時点での最新の起動時間情報を読み直さない不具合を修正
