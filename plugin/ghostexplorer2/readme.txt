...
・ゴーストを数十体インストールしている場合、初回起動にかなり時間がかかる場合がありますが、正常です。
  （顔画像の生成のために時間がかかっています）
  2回目以降の起動では、1回目に生成した画像をキャッシュして使いまわすため、もう少し起動が早くなると思います。
...
・ゴーストエクスプローラ通を起動したゴーストを他のゴーストに切り替えるか、もしくは終了すると、
  それ以降はゴーストエクスプローラ通を起動しなおすまで、ゴーストの切り替えが実行できなくなります。（呼び出しは可能です）
